  ...
  void HardFault_c_handler(uint32_t * p_stack_address) {
    ...
    NRF_LOG_FINAL_FLUSH();
      ...
      if (p_stack != NULL) {
        ...
        NRF_LOG_ERROR("  R0:  0x%08X  R1:  0x%08X  R2:  0x%08X  R3:  0x%08X", p_stack->r0, p_stack->r1, p_stack->r2, p_stack->r3);
        NRF_LOG_ERROR("  R12: 0x%08X  LR:  0x%08X  PSR: 0x%08X", p_stack->r12, p_stack->lr, p_stack->psr);
        }
      else {
