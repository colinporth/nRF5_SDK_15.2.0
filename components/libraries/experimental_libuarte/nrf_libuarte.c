...
#define PPI_GROUP_SETUP(_ch, _group, _en_tsk, _dis_tsk)                        \
  ret = nrfx_ppi_channel_include_in_group(m_ppi_channels[_ch], m_ppi_groups[_group]); \
  if (ret != NRF_SUCCESS)                                                \
    return NRF_ERROR_INTERNAL;                                         \
...
void UART_DRV_IRQHandler() {
  ...
  if (nrf_uarte_event_check (UART_DRV_UARTE, NRF_UARTE_EVENT_ENDRX)) {
    ...
    NRF_LOG_DEBUG ("(evt) RX dma_cnt:%d, endrx_cnt:%d, stop_cnt:%d", dma_amount, endrx_byte_cnt, stop_byte_cnt);
    ...
    if (dma_amount || pin_amount) {
      ...
      NRF_LOG_DEBUG ("RX END chunk0:%d, chunk1:%d, data[0]=%d %d",
                     chunk0, chunk1, mp_cur_rx[0], mp_cur_rx[1]);
      nrf_libuarte_evt_t evt = {
      ...
      if (chunk1 || ((dma_amount == m_chunk_size) && (endrx_byte_cnt == stop_byte_cnt))) {
...
void nrf_libuarte_rx_buf_rsp (uint8_t* p_data, size_t len) {
  ...
  if (mp_next_rx == NULL) {
    ...
    NRF_LOG_DEBUG ("RX buf response (mp_next_rx not NULL:0x%08X), Provided buffer:0x%08X", mp_next_rx, p_data);
    mp_next_next_rx = p_data;
    }
  }
//}}}
//{{{
void nrf_libuarte_rx_stop() {
